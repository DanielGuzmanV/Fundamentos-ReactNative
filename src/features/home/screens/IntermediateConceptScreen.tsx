...
import { intermediateConcepts } from '../data/concepts/intermediate_concepts';

export default function IntermediateConceptScreen() {
  // Obtenemos los datos correspondientes al nivel básico
  const intermediateData = intermediateConcepts;

  return (
    <ViewCustom style={styles.container}>
      <ScrollView contentContainerStyle={styles.scrollContent}>
        <TextCustom style={styles.title}>{intermediateData?.title}</TextCustom>
        <TextCustom style={styles.description}>{intermediateData?.description}</TextCustom>

        {/* Aquí listaremos los temas individuales (Tema 1: FlatList, etc.) */}
        <View style={styles.itemsContainer}>
          {intermediateData?.items.map((item, index) => (
